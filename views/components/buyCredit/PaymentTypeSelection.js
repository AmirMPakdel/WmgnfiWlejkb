        ...
        return(
            <div className={styles.con+" md_card_shd bgw"}>
                ...
                <div className={styles.wrapper1}>
                    ...
                    <div className={styles.credit_sec+" bdyt"}>{"مقدار اعتبار باقی مانده : "}<span className="tc2 bdyt">{priceFormat(ps.credit)+" تومان"}</span></div>
                    
                    <div className={styles.credit_sec+" bdyt"}>{"مقدار صندوق درآمد : "}<span className="tc2 bdyt">{priceFormat(ps.incomes)+" تومان"}</span></div>

                </div>

                <div className={styles.item_con+" amp_btn "+((ps.payment_type == 1)?"tbc2 ":"blc2 ")}
                onClick={()=>this.onPaymentType(1)}>
                    <Radio checked={ps.payment_type == 1}/>
                    <div className={styles.item_text+" bdyt"}>{"خرید اینترنتی"}</div>
                </div>

                <div className={styles.item_con+" amp_btn "+((ps.payment_type == 2)?"tbc2 ":"blc2 ")}
